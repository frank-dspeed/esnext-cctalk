 ...
 * @returns 
 */
const getCompletPayloadAndPreservedDataBuffer = payload => {
  const moreThen2bytes = payload.length > 1;
  
  if (!moreThen2bytes) {
    return { 
      cctalkPayload: new Uint8Array, 
      preservedDataBuffer: payload 
    }
  }
  
  const dataLength = payload[1];
  const endOfChunk = 5 + dataLength;
  const isCompletPayload = payload.length >= endOfChunk;
  const preservedDataBuffer = isCompletPayload 
    ? payload
      .slice(endOfChunk, payload.length)
    : payload;
...
export const OnPayloadComplet = ( maxDelayBetweenBytesMs = 50 ) => {
  ...
  onCompletPayloadInstance._transform = ( buffer, destination ) => {
    ...
    const { cctalkPayload, preservedDataBuffer} = getCompletPayloadAndPreservedDataBuffer(Uint8ArrayView);
    onCompletPayloadInstance.preservedDataBuffer = preservedDataBuffer;
    if (!cctalkPayload.length) {
        return
    }
    ...
    const shouldReadNextPayload = preservedDataBuffer.length > 1;
